...
public sealed class LSLogger {
    ...
    public bool? SetSourceStatus((string sourceID, bool? isEnabled)? source, bool dontUpdateSources = false) {
        // If source is null, treat as enabled (no source filtering applied)
        if (source == null) return true;
        string sourceID = source.Value.sourceID;
        // source.isEnabled overrides existing value when provided
    ...
    public bool IsSourceEnabled(string sourceID) {
        ...
        // Default for unknown sources: enabled
        return _sources.TryGetValue(sourceID, out var isEnabled) ? (isEnabled ?? true) : true;
    }
