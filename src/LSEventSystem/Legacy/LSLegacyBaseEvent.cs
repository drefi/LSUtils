...
/// </summary>
public abstract class LSLegacyBaseEvent : ILSLegacyMutableEvent {
    ...
    /// </summary>
    public LSLegacyDispatcher Dispatcher { get; protected set; } = LSLegacyDispatcher.Singleton;
    ...
    public LSLegacyEventCallbackBuilder<TEventType> WithCallbacks<TEventType>(LSLegacyDispatcher dispatcher)
        where TEventType : ILSEvent {
        if (IsBuilt) throw new LSException("Event has already been built.");

        Dispatcher = dispatcher;
        IsBuilt = true;
        return new LSLegacyEventCallbackBuilder<TEventType>((TEventType)(object)this, Dispatcher);
    }
    ...
    public bool Dispatch(LSLegacyDispatcher dispatcher) {
        if (dispatcher == null)
    ...
    public bool Dispatch() {
        return Dispatch(LSLegacyDispatcher.Singleton);
    }
